...
/**
 * A class which allows animations. Allows the {@code paint(java.awt.Graphics2D)} method to be overridden
 * @author Julian Scicluna
 * @version 1.0
*/
public abstract class AbstractAnimation {
    //private static ExecutorService animationsThreadPool = Executors.newFixedThreadPool(5);
    //protected boolean isComplete = false;
    private static boolean hasBeenInitialised = false;
    private boolean isPaused = false, isActive = false;
    //protected long initTime, duration, elapsedTimeBeforeLatestPause = 0;
    ...
    /*public ExecutorService setExecutorService(ExecutorService newService, boolean destroyOld) {
        if (hasBeenInitialised) {
            throw new IllegalStateException("Cannot change the ExecutorService of AbstractAnimations");
        } else if (newService == null) {
            throw new NullPointerException("The new ExecutorService cannot be null!");
        } else {
            ExecutorService oldEs = animationsThreadPool;
            animationsThreadPool = newService;
            if (destroyOld) {
                oldEs.shutdownNow();
            }
            return oldEs;
